(() => {
  interface SiteAdapter {
    turnSelector: string;
    getRole: (el: Element) => 'user' | 'assistant';
    getText: (el: Element) => string | undefined;
  }

  const SITE_ADAPTERS: Record<string, SiteAdapter> = {
    'chatgpt.com': {
      turnSelector: '[data-message-author-role]',
      getRole: el => (el as HTMLElement).dataset.messageAuthorRole as 'user' | 'assistant',
      getText: el => (el.querySelector('.markdown, .text-base') as HTMLElement | null)?.innerText?.trim()
    },
    'chat.openai.com': {
      turnSelector: '[data-message-author-role]',
      getRole: el => (el as HTMLElement).dataset.messageAuthorRole as 'user' | 'assistant',
    ...
    'gemini.google.com': {
      ...
      getText: el => (el.querySelector('.query-text, .response-content') as HTMLElement | null)?.innerText?.trim()
    },
    'www.perplexity.ai': {
      turnSelector: "[class*='UserMessage'], [class*='AnswerBody']",
      getRole: el => el.className.includes('UserMessage') ? 'user' : 'assistant',
      getText: el => (el as HTMLElement).innerText?.trim()
    },
    'grok.com': {
    ...
    'you.com': {
      ...
      getRole: el => (el as HTMLElement).dataset.testid?.includes('human') ? 'user' : 'assistant',
      getText: el => (el as HTMLElement).innerText?.trim()
    },
    'claude.ai': {
      ...
      getText: el => {
        const html = el as HTMLElement;
        const candidate = html.querySelector('.markdown, .text-base, .query-text, .response-content, [class*="content"], [class*="message"]') as HTMLElement | null;
        const text = candidate?.innerText?.trim() || html.innerText?.trim();
        return text ? text.replace(/\b(Copy|Copy code|Regenerate|Retry|Edit|Thumbs up|Thumbs down)\b/gi, '').replace(/\s+/g, ' ').trim() : undefined;
      }
    }
  };
  ...
  function cleanText(text: string): string {
    return text
      .replace(/\b(Copy|Copy code|Regenerate|Retry|Edit|Thumbs up|Thumbs down)\b/gi, '')
      .replace(/\s+/g, ' ')
      .trim();
  }

  function inferRole(el: Element): 'user' | 'assistant' {
    const html = el as HTMLElement;
    const role = html.getAttribute('data-message-author-role');
  ...
  function normalizePlatform(hostname: string): 'chatgpt' | 'claude' | 'gemini' | 'deepseek' {
  ...
  function makeGenericAdapter(): SiteAdapter {
    return {
      ...
      getText: (el) => {
        ...
        for (const selector of selectors) {
          const candidate = html.querySelector(selector) as HTMLElement | null;
  ...
  function getConversationId(): string | null {
    const pathMatch = window.location.pathname.match(/\/c\/([a-z0-9-]+)/i)
      || window.location.pathname.match(/\/chat\/([a-z0-9-]+)/i)
      || window.location.pathname.match(/\/conversation\/([a-z0-9-]+)/i);
  ...
  function extractTurnText(el: Element, adapter: SiteAdapter): string | undefined {
  ...
  function scrapeMessages(): { role: 'user' | 'assistant'; content: string; timestamp: number }[] {
    ...
    turns.forEach((turn, idx) => {
      ...
      messages.push({
        ...
        timestamp: Date.now() - (turns.length - idx) * 1000
      });
    });
