...
(() => {
  ...
  async function decryptText(encryptedComposite, key) {
    try {
  ...
  // src/services/db.ts
  var AIContextBridgeDatabase = class extends import_wrapper_default {
    projects;
    conversations;
    messages;
    constructor() {
  ...
  var db = new AIContextBridgeDatabase();
  async function getActiveCryptoKey() {
  ...
  async function decryptTitle(title, key) {
    ...
    return title.replace(/^enc:/, "");
  }
  async function decryptMessageContent(content, key) {
  ...
  async function encryptIfNeeded(text, key) {
    if (key) {
  ...
  async function saveCapturedConversation(conversationId, title, platform, url, rawMessages, projectId = null) {
  ...
  async function getConversationMessages(conversationId) {
    const key = await getActiveCryptoKey();
    const rawMsgs = await db.messages.where("conversationId").equals(conversationId).sortBy("index");
    const decryptedMsgs = [];
  ...
  function buildFallbackTitle(site, text, url) {
  ...
  async function saveTurnCapture(payload) {
    ...
    const duplicateTurn = existingMessages.some(
      (message) => message.role === payload.role && message.content === sanitizedText
    );
    if (duplicateTurn) {
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message.type === "SAVE_TURN") {
      saveTurnCapture(message.payload).then(() => {
    ...
    if (message.type === "MANUAL_SAVE_CHAT") {
      const { id, title, platform, url, messages } = message.data;
      saveCapturedConversation(id, title, platform, url, messages).then(() => {
        sendResponse({ success: true });
      }).catch((error) => {
